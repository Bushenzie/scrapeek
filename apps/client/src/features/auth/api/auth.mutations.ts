...
export const useCreateApiKey = () =>
	useMutation({
		mutationFn: async (name: string) => {
			...
			if (response.error)
				throw new Error("There was an error during creation of an API Key");
...
export const useDeleteApiKey = () =>
	useMutation({
		mutationFn: async (id: string) => {
			...
			if (response.error)
				throw new Error("Something went wrong during deletion of an API key");
		},
		meta: {
			invalidatesQuery: apiKeyQueryKeys.lists(),
		},
...
export const useUpdateApiKey = () =>
	useMutation({
		mutationFn: async ({ id, name }: { id: string; name: string }) => {
			...
			if (response.error)
				throw new Error("Something went wrong during update an API key");
		},
		meta: {
