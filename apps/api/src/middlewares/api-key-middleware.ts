...
export const apiKeyMiddleware = createMiddleware(async (c, next) => {
  ...
  if (!apiKey) throw new StatusError("No API Key provided", StatusCodes.UNAUTHORIZED)
  ...
  if (!valid) {
    throw new StatusError(error?.message ?? "API Key is invalid", StatusCodes.UNAUTHORIZED)
  }
