export const getValueFromFlatPath = <T extends object>(data: T, path: string) => {
  ...
  for (const segment of segments) {
    ...
    if (isArray) {
      ...
      const remainingSegmentsPath = segments.slice(index + 1, segments.length).join(".")
    ...
    if (isIndexed) {
      ...
      current = (current === undefined ? data : current)[clearedSegment][specifiedIndex]
