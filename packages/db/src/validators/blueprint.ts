...
import { z } from "zod"
import { BLUEPRINT_HTTP_METHODS, BlueprintType, DATABASE_FIELDS } from "../lib/constants"
...
const variantsOf = <Shape extends z.core.$ZodLooseShape>(base: z.ZodObject<Shape>) =>
  [
    base.extend({
      type: z.literal("string"),
      removeNewLines: z.boolean().default(true).optional(),
    }),
    base.extend({ type: z.literal("number") }),
    base.extend({
      type: z.literal("boolean"),
      condition: z.object({
        operation: z.enum(["equals", "notEquals"]),
        to: z.union([z.string(), z.number()]),
      }),
    }),
  ] as const

const depth2ElementVariantSchema = z.discriminatedUnion("type", variantsOf(elementBaseSchema))
const depth1ElementVariantSchema = z.discriminatedUnion(
  "type",
  variantsOf(elementBaseSchema.extend({ crawl: depth2ElementVariantSchema.array().optional() })),
)
const elementVariantSchema = z.discriminatedUnion(
  "type",
  variantsOf(elementBaseSchema.extend({ crawl: depth1ElementVariantSchema.array().optional() })),
)
...
const elementsSchema = z.object({
  ...
  container: z.object({
    selector: z.string(),
    elements: elementVariantSchema.array(),
  }),
})
...
const apiConfigSchema = z.object({
  fields: z.array(elementBaseSchema.pick({ key: true, selector: true })),
...
export const apiBlueprintWithRelationsSchema = baseSelectSchemaWithRelationsSchema.extend({
  type: z.literal(BlueprintType.API),
  config: apiConfigSchema,
})

export const staticBlueprintWithRelationsSchema = baseSelectSchemaWithRelationsSchema.extend({
  type: z.literal(BlueprintType.Static),
  config: staticConfigSchema,
})

export const dynamicBlueprintWithRelationsSchema = baseSelectSchemaWithRelationsSchema.extend({
  type: z.literal(BlueprintType.Dynamic),
  config: dynamicConfigSchema,
})
...
export type BlueprintWithRelations = z.infer<typeof blueprintWithRelationsSchema>
export type APIBlueprintWithRelations = z.infer<typeof apiBlueprintWithRelationsSchema>
export type StaticBlueprintWithRelations = z.infer<typeof staticBlueprintWithRelationsSchema>
export type DynamicBlueprintWithRelations = z.infer<typeof dynamicBlueprintWithRelationsSchema>
...
export type EditableStaticBlueprint = z.infer<typeof staticUpdateBlueprintSchema>
export type EditableDynamicBlueprint = z.infer<typeof dynamicUpdateBlueprintSchema>
...
export type InsertDynamicBlueprint = z.infer<typeof dynamicInsertBlueprintSchema>

export type ContainerElementSelector = z.infer<typeof elementsSchema>
export type ElementSelector = z.infer<typeof elementVariantSchema>
